...
/**
 * The practical detail a visitor wants after clicking a listing.
 *
 * Every sentence here is derived from data the site already holds — the date,
 * the country's holiday calendar, and the other listings around it. Nothing is
 * said about the act, the venue or the show itself, because the site knows
 * nothing about them and guessing would be worse than saying less.
 *
 * @param {object} event an annotated event
 * @param {{countryName: string, today: string, sameCityThatWeek?: number,
 *          nextDayHoliday?: {name: string} | null}} context
 * @returns {string[]} sentences, in reading order
 */
export function describeEvent(event, context) {
  ...
  // 1. When and where, stated plainly.
  const where = [event.venue, event.city].filter(Boolean).join(', ');
  if (event.time && where) lines.push(`${when}, from ${event.time}, at ${where}.`);
  else if (event.time) lines.push(`${when}, from ${event.time}.`);
  else if (where) lines.push(`${when} at ${where}. No start time has been published yet.`);
  else lines.push(`${when}. No venue or start time has been published yet.`);

  // 2. What the date means for getting there and back — the part a listing
  //    never tells you, and the reason these pages sit beside a calendar.
  if (event.holiday) {
    lines.push(
      `This date is ${event.holiday.name}, a public holiday in ${context.countryName}. ` +
        `Public transport often runs a reduced timetable on public holidays and many shops and ` +
        `offices close, so allow more time than you would on an ordinary ${event.weekday}.`,
    );
  } else if (context.nextDayHoliday) {
    lines.push(
      `The following day is ${context.nextDayHoliday.name}, a public holiday in ` +
        `${context.countryName} — so a late finish costs you nothing the next morning.`,
    );
  } else if (event.weekend) {
    lines.push(`It falls on a ${event.weekday}, so nobody needs to book time off for it.`);
  } else {
    lines.push(
      `It falls on a ${event.weekday}, an ordinary working day in ${context.countryName}.`,
    );
  }

  // 3. How far off it is, in the terms people actually think in.
  const away = Math.round((date.getTime() - parseISO(context.today).getTime()) / 86400000);
  if (away === 0) lines.push('That is today.');
  else if (away === 1) lines.push('That is tomorrow.');
  else if (away < 7) lines.push(`That is ${away} days away.`);
  else if (away < 14) lines.push('That is a little over a week away.');
  else if (away < 70) lines.push(`That is about ${Math.round(away / 7)} weeks away.`);
  else lines.push(`That is about ${Math.round(away / 30.44)} months away.`);

  // 4. Whether the trip is worth stretching.
  if (context.sameCityThatWeek > 0 && event.city) {
    lines.push(
      context.sameCityThatWeek === 1
        ? `One other listing in ${event.city} falls in the same week.`
        : `${context.sameCityThatWeek} other listings in ${event.city} fall in the same week.`,
    );
  }
...
/**
 * Context for describeEvent(), computed once per country rather than per row.
 * @param {Array} events the country's full annotated listing set
 */
export function describeContext(events, { countryName, today, holidaysByDate }) {
  const weekOf = (date) => Math.floor(parseISO(date).getTime() / (7 * 86400000));

  // How many other listings share a city and a week with each event.
  const crowd = new Map();
  for (const event of events) {
    if (!event.city) continue;
    const key = `${event.city}|${weekOf(event.date)}`;
  ...
  return (event) => ({
    countryName,
    today,
    sameCityThatWeek: event.city ? (crowd.get(`${event.city}|${weekOf(event.date)}`) || 1) - 1 : 0,
    nextDayHoliday: holidaysByDate.get(iso(addDays(parseISO(event.date), 1))) || null,
  });
}

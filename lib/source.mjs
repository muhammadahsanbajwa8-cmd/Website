...
import { fallbackCountries, fallbackHolidays, hasFallback } from './fallback.mjs';
...
export function createSource(options) {
  ...
  return {
    ...
    async holidays(year, code) {
      ...
      if (rows.length) return { holidays: rows, origin };

      const computed = fallbackHolidays(code, year);
      if (computed && computed.length) {
        stats.computed += 1;
        return { holidays: computed, origin: 'computed' };
      }
      stats.missing += 1;
      return { holidays: [], origin: 'none' };
    },

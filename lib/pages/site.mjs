 ...
 */
/** The questions the home page offers as a way into the assistant. */
const ASK_EXAMPLES = [
  'How many working days until 24 December in the United States?',
  'When can US, GB and IN all meet?',
  'I have 14 days of annual leave next year in the United Kingdom',
];
...
<section class="section">
  <div class="wrap">
    <div class="section__head">
      <h2>Or just ask</h2>
      <a href="${url.assistant()}">Ask a question →</a>
    </div>
    <p>How many working days until the deadline. Whether the 25th is a day off where your supplier
      is. Where to put fourteen days of leave so they buy six weeks. Type the question the way you
      would say it and the answer is worked out in your browser, from these same tables — no
      account, no waiting, nothing sent anywhere.</p>
    <ul class="chips">${ASK_EXAMPLES.map(
      (question) =>
        `<li><a class="chip" href="${url.assistant()}?q=${encodeURIComponent(question)}">${esc(
          question,
        )}</a></li>`,
    ).join('')}</ul>
  </div>
...
export function renderAbout({ countries, years, generatedAt, computedCountries, coreCount = 0 }) {
...
<section class="section">
  <div class="wrap prose">
    ...
    <p>Where the API has no entry for a country and year, dates are computed from that country's
      statutory rules instead. There are two tiers of that, and the pages tell you which one you are
      reading:</p>
    <ul>
      <li><strong>Full rule sets</strong> — ${computedCountries.join(', ')}. Complete national lists
        including each country's own law on what happens when a holiday falls at a weekend.</li>
      ${
        coreCount
          ? `<li><strong>Core rule sets</strong> — ${coreCount} further countries, carrying the
        principal national holidays only. Any page built from one says so, because a working-day
        count from a partial list is an upper bound rather than an answer.</li>`
          : ''
      }
    </ul>
    <p>The rules cover fixed dates, "nth weekday of the month" holidays, offsets from Easter Sunday
      (Gregorian, by the Meeus/Jones/Butcher algorithm, and Orthodox, by the Julian computus), and
      dates in the Islamic calendar. No per-year date lists are stored anywhere, so the computed
      dates stay correct for any year you ask for.</p>
    <p>Islamic dates are the one place this site publishes an approximation. Eid al-Fitr, Eid al-Adha
      and the rest begin on the sighting of the new moon, which varies by country and is confirmed
      only days ahead; no algorithm can settle them in advance. What you see is the arithmetic
      calendar, which lands within a day or so, and every such date is labelled
      <em>estimated</em> wherever it appears.</p>
    ...
    <h2>Concerts, comedy and live events</h2>
    <p>Where event listings appear, they come from the
      <a href="https://developer.ticketmaster.com" rel="noopener">Ticketmaster Discovery API</a> and
      are refreshed on every nightly rebuild, limited to the next few months. Coverage follows that
      source — strong in North America, the UK, Ireland, Australia and much of western Europe, thin
      or absent elsewhere — so a country with no listings gets no event page rather than an empty
      one. Nothing is generated to fill a gap.</p>
    <p>Every listing is matched against that country's public holiday calendar, which is the reason
      these pages sit next to the calendar at all: a show on a public holiday means a different
      transport timetable, different opening hours and different prices. Ticket links go to the
      seller. This site sells nothing, takes no booking, and earns nothing from a click-through.</p>
  ...
  <div class="wrap prose">
    ...
    <ul>
      ...
      <li><strong>Working-day counts are a default, not your contract.</strong> They assume a
        ...
        differ.</li>
      <li><strong>Event listings are a snapshot, not a diary.</strong> They show what one ticket
        seller had on sale when the site was last built. Independent venues, local promoters and
        anything sold elsewhere do not appear; times and venues change after publication; and an
        absent listing means nothing about whether something is happening.</li>
    </ul>
...
<section class="section">
  <div class="wrap prose">
    ...
    <p>Where event pages appear, the listings were fetched when the site was built and are baked
      into the page as text — your browser makes no request to the ticket seller while you read.
      Following a ticket link takes you to that seller's own site, where their privacy policy
      applies, not this one. No images, trackers or embeds are loaded from them, and this site earns
      nothing from a click-through.</p>

...
/**
 * What is actually on while you are there. Only rendered when the build had
 * event data for at least one of the two countries.
 */
function whatsOn(result) {
  const { a, b } = result;
  if (!a.events && !b.events) return '';

  const column = (country) => {
    if (!country.events || !country.events.list.length) {
      return `<div>
        <h3>${esc(country.flag)} ${esc(country.name)}</h3>
        <p class="note">No listings for ${esc(country.name)} in this build.</p>
      </div>`;
    }
    return `<div>
      <h3>${esc(country.flag)} ${esc(country.name)}</h3>
      <p class="note mono">${country.events.counts.concerts} concerts · ${
        country.events.counts.comedy
      } comedy · ${country.events.counts.all} listed</p>
      <ul class="breaks">${country.events.list
        .slice(0, 5)
        .map(
          (event) => `<li>
        <span class="breaks__days mono">${esc(event.date.slice(5))}</span>
        <span class="breaks__when">${esc(event.name)}</span>
        <span class="breaks__why">${[event.venue, event.city].filter(Boolean).map(esc).join(', ')}${
          event.holiday ? ` — on ${esc(event.holiday)}` : ''
        }</span>
      </li>`,
        )
        .join('')}</ul>
      <p class="note"><a href="${url.countryEvents(country.code, 'all')}">All ${esc(
        country.name,
      )} listings →</a></p>
    </div>`;
  };
...
<section class="section">
  <div class="wrap">
    <div class="section__head">
      <h2>What's on while you are there</h2>
      <p class="note">next dates on sale</p>
    </div>
    <div class="split">
      ${column(a)}
      ${column(b)}

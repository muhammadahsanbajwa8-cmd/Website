...
const COURSE = 86, PLATE = 26, WALL_ALLOW = 115, MIN_DIM = 900;
...
function stack(rooms, band, alongY, outerLow = true) {
  ...
  for (let i = 0; i < rooms.length;) {
    ...
    if (thin && !r.solo && i + 1 < rooms.length) {
      ...
      // Two rooms that both need daylight can never share a slice: one of
      // them would be against the passage with no external wall, which is
      // how a bedroom ends up in the middle of the plan.
      const bothNeedLight = needsLight(r) && needsLight(n);
      if (!n.solo && ns > 0 && depth / ns > 2.2 && !bothNeedLight &&

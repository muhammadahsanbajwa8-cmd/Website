...
function buildProgram(a) {
  ...
  // room that could hold the one fitting it is named after. The Python
  // derives this from the fittings catalogue in export/fixtures.py; the page
  // carries no fittings, so it is stated here and the comment is what says
  // where it came from.
  rooms.push(R("wc", "WC", "wc", 1.8, 1180, { storey: 0, priority: 3 }));
...
function placeFront(front, strip, over) {
  ...
  // with nothing to put in it was left as floor no room covers: up to 17 m²
  // inside the outline with no room, no label and no floor, on every lot 12 m
  // wide or under. The width goes to whatever is already against it.
  if (!right.length && rightW > 0) {
    if (porticoRect && porticoRect.x >= slotX + slotW)
      porticoRect = { ...porticoRect, w: porticoRect.w + rightW };
    else slotW += rightW;
    rightW = 0;
  } else if (!left.length && leftW > 0) {
    if (porticoRect && porticoRect.x + porticoRect.w <= slotX)
      porticoRect = { ...porticoRect, x: strip.x, w: porticoRect.w + leftW };
    else { slotX -= leftW; slotW += leftW; }
    leftW = 0;
  }
